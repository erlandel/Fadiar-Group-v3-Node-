
export const privacyPolicy=`
1. IDENTIDAD DEL RESPONSABLE
El responsable del tratamiento de los datos personales recogidos a través del sitio web Grupo Fadiar, disponible en https://sample1.grupofadiar.com/ (en adelante, el “Sitio Web”), es Grupo Fadiar.
Si tienes cualquier duda relacionada con el tratamiento de tus datos personales, puedes contactar con nosotros a través del correo electrónico atencionalcliente@grupofadiar.com o del teléfono +53 63513228, así como mediante los canales de contacto indicados en nuestro Sitio Web oficial.

2. DATOS PERSONALES QUE RECABAMOS
En función de cómo uses el Sitio Web, podemos recabar las siguientes categorías de datos:
- Datos de identificación: nombre, apellidos, nombre de usuario, identificadores internos.
- Datos de contacto: correo electrónico, número de teléfono u otros medios de contacto que facilites.
- Datos de acceso y uso: credenciales, historial de navegación en el Sitio Web, fecha y hora de acceso, secciones visitadas y configuración seleccionada.
- Datos de comunicaciones: mensajes que nos envíes a través de formularios de contacto, soporte o canales de atención.
En ningún caso recabamos más datos de los estrictamente necesarios para cada finalidad indicada en esta Política.

3. FINALIDADES DEL TRATAMIENTO
Usamos tus datos personales para las siguientes finalidades:
- Gestionar tu registro y cuenta de usuario en el Sitio Web.
- Prestarte los servicios y funcionalidades disponibles en el Sitio Web.
- Gestionar comunicaciones relacionadas con el servicio, notificaciones técnicas y avisos importantes.
- Atender solicitudes de soporte, consultas o reclamaciones que nos hagas llegar.
- Mejorar la calidad, funcionamiento y seguridad del Sitio Web, incluyendo el análisis del uso y el rendimiento.
- Cumplir obligaciones legales, regulatorias o requerimientos de autoridades competentes.
- En su caso, enviarte comunicaciones comerciales electrónicas relacionadas con nuestros servicios, siempre que lo hayas autorizado o exista base legal para ello.

4. BASE LEGAL DEL TRATAMIENTO
Tratamos tus datos personales sobre las siguientes bases jurídicas:
- Ejecución de un contrato: para gestionar tu registro, proporcionar las funcionalidades del Sitio Web y atender tus solicitudes.
- Interés legítimo: para mejorar nuestros servicios, garantizar la seguridad del Sitio Web, prevenir el fraude y gestionar comunicaciones relacionadas con el servicio que sean necesarias.
- Cumplimiento de obligaciones legales: cuando debamos conservar o facilitar datos a autoridades públicas o cumplir normativas aplicables.
- Consentimiento: para el envío de comunicaciones comerciales o para el tratamiento de determinados datos cuando así lo requiera la normativa. En cualquier momento podrás retirar tu consentimiento sin que ello afecte a la licitud del tratamiento previo.

5. CONSERVACIÓN DE LOS DATOS
Conservaremos tus datos personales durante el tiempo estrictamente necesario para cumplir con las finalidades descritas y, posteriormente, durante los plazos de prescripción legal aplicables.
Cuando ya no sean necesarios, los datos serán bloqueados y, una vez finalicen los plazos legales correspondientes, serán eliminados de forma segura.

6. COMUNICACIÓN DE DATOS A TERCEROS
Podemos compartir tus datos personales únicamente en los siguientes casos:
- Obligaciones legales: cuando exista una obligación legal o requerimiento de autoridades competentes, tribunales u organismos reguladores.
- Operaciones corporativas: en caso de una fusión, adquisición, reestructuración o venta de activos, en cuyo caso los datos podrán ser transferidos a la entidad sucesora, respetando en todo momento la normativa de protección de datos.
En ningún caso vendemos tus datos personales a terceros.

7. TRANSFERENCIAS INTERNACIONALES
Si para la prestación de nuestros servicios necesitamos realizar transferencias internacionales de datos fuera de tu país de residencia, adoptaremos las garantías adecuadas para proteger tu información, de acuerdo con la normativa vigente en materia de protección de datos. Esto puede incluir el uso de cláusulas contractuales tipo u otros mecanismos reconocidos por la autoridad de control competente.

8. DERECHOS DE LOS USUARIOS
Puedes ejercer, en los casos y con el alcance previstos por la normativa aplicable, los siguientes derechos sobre tus datos personales:
- Derecho de acceso: conocer qué datos tuyos tratamos.
- Derecho de rectificación: solicitar la corrección de datos inexactos o incompletos.
- Derecho de supresión: pedir la eliminación de tus datos cuando, entre otros motivos, ya no sean necesarios para los fines para los que fueron recogidos.
- Derecho de oposición: oponerte al tratamiento de tus datos cuando se base en intereses legítimos, incluida la elaboración de perfiles.
- Derecho de limitación del tratamiento: solicitar la limitación del tratamiento de tus datos en determinadas circunstancias.
- Derecho a la portabilidad: recibir tus datos personales en un formato estructurado, de uso común y lectura mecánica, y transmitirlos a otro responsable cuando el tratamiento se base en el consentimiento o en un contrato y se efectúe por medios automatizados.
Para ejercer tus derechos, puedes dirigirte a nuestro canal de contacto indicado en el Sitio Web, indicando claramente tu identidad y el derecho que deseas ejercer. Podremos solicitar información adicional para verificar tu identidad cuando sea necesario.Además, si consideras que tus datos no han sido tratados adecuadamente, puedes presentar una reclamación ante la autoridad de control competente en materia de protección de datos.

9. SEGURIDAD DE LA INFORMACIÓN
Aplicamos medidas técnicas y organizativas apropiadas para proteger tus datos personales frente a pérdidas, accesos no autorizados, alteraciones o divulgaciones indebidas. No obstante, ningún sistema es completamente seguro, por lo que no podemos garantizar la seguridad absoluta de la información.
Te recomendamos que también adoptes medidas razonables para proteger tus datos, como mantener tus credenciales confidenciales y utilizar contraseñas robustas.

10. USO POR MENORES DE EDAD
El Sitio Web no está dirigido a menores de 18 años. Si eres menor de edad, debes utilizar el Sitio Web con la autorización y supervisión de tus padres o tutores legales.
Si tenemos conocimiento de que hemos recopilado datos personales de un menor sin el consentimiento exigido por la normativa, adoptaremos las medidas razonables para eliminarlos.

11. CAMBIOS EN ESTA POLÍTICA DE PRIVACIDAD
Podemos actualizar esta Política de Privacidad cuando resulte necesario para reflejar cambios en la normativa, en nuestros servicios o en la forma en que tratamos tus datos. En caso de cambios significativos, te informaremos a través del Sitio Web o por otros medios equivalentes.
Te recomendamos revisar periódicamente esta Política para estar informado sobre cómo protegemos tu información.

12. CONTACTO
Si tienes preguntas sobre esta Política de Privacidad o sobre cómo tratamos tus datos personales, puedes ponerte en contacto con nosotros a través del correo electrónico atencionalcliente@grupofadiar.com, del teléfono +53 63513228 o utilizando el canal de soporte indicado en el Sitio Web.
`
